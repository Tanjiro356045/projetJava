...
public class UtilisateurDAOJdbcImpl implements UtilisateurDAO{
	...
	public List<Utilisateur> selectAll() {
		...
		try (Connection cnx = ConnectionProvider.getConnection()) {
			...
			while (rs.next()) {
				listeUtilisateur.add(new Utilisateur(rs.getInt("no_utilisateur"), 
													 rs.getString("pseudo"), 
													 rs.getString("nom"), 
													 rs.getString("prenom"), 
													 rs.getString("email"), 
	...
	public void update(Utilisateur data) {
		...
		try (Connection cnx = ConnectionProvider.getConnection()){
			...
			pstmt.setString(5, data.getNoTelephone());
			pstmt.setString(6, data.getRue());
			pstmt.setString(7, data.getCodePostal());
			pstmt.setString(8, data.getVille());
			pstmt.setString(9, data.getMotDePasse());
			pstmt.setInt(10, data.getCredit());
			pstmt.setBoolean(11, data.isAdministrateur());
			pstmt.setInt(12, data.getNoUtilisateur());
			pstmt.executeUpdate();
